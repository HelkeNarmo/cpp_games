...
// 暂时禁用音频功能，因为type_tag项目没有自己的音频文件
...
bool AudioManager::loadAudioAssets() {
    ...
    //     return false;
    // }
    SDL_Log("Audio loading skipped - no audio files available for type_tag project");
    return true;
}
...
void AudioManager::playBGM(int idx, float volume01) {
    ...
    // }   
    // 音频功能暂时禁用
    SDL_Log("BGM playback skipped - audio disabled");
}
...
void AudioManager::playHurt(float volume01) {
    ...
    // }
    // 音频功能暂时禁用
    SDL_Log("Hurt sound playback skipped - audio disabled");
}
